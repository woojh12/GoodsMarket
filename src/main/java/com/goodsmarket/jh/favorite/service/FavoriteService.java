...
public class FavoriteService {
	...
	// 즐겨찾기 담기 Service
	public int addFavorite(int usedTradeId, int userId)
	{
		return favoriteRepository.insertFavorite(usedTradeId, userId);
	}
	
	// 즐겨찾기 체크 Service
	public int getFavoriteCount(int userTradeId, int userId)
	{
		return favoriteRepository.countFavorite(userTradeId, userId);
	}
	
	// 즐겨찾기 한개만 삭제 Service
	public int removeFavoriteCount(int userTradeId, int userId)
	{
		return favoriteRepository.deleteFavorite(userTradeId, userId);
	}
	
	// 즐겨찾기 모두 삭제 
	public int removeAllFavorite(int userId)
	{
		return favoriteRepository.deleteAllFavorite(userId);
	}
	
	// 즐겨찾기 선택한 모든 인원수 조회 service
	public int getAllFavoriteCount(int userTradeId)
	{
		return favoriteRepository.countAllFavorite(userTradeId);
	}
	
	//241110 수정
	// 즐겨찾기에 담긴 게시글 정보 가져오기 service
	public List<BoardDTO> getUserFavoriteList(int userId)
	{
		// 사용자가 누른 즐겨찾기 버튼 게시물의 id 값 조회
		int[] usedTradeId = favoriteRepository.selectUserFavoriteList(userId);
		...
		for(int i = 0; i < usedTradeId.length; i++)
		{
			...
			// 각 게시물의 첫번째 이미지 추출
			List<FileImage> fileImage = fileImageRepository.selectAllFileImage(usedTradeId[i]);
